...
const DATA = {
  meta: {
    cohort: "WEMBA 52",
    lastUpdated: "2026-08-06",   // ← 업데이트할 때마다 이 날짜만 바꾸세요 (YYYY-MM-DD)
    author:   "Sally",
    tzLabel:  "PT",
  ...
  whatsNew: [
    ...
    { date:"2026-08-05", kr:"🎯 BEPP 기말 — Pras가 후반부 세션 4개 전부 게시(S8 게임이론·S9 효용극대/보험·S10 워런티/정보비대칭·S11 경매 → L9–17 커버 완료) · ⚠️ **일 8/9 저녁 전범위 리뷰 세션**(평소 화 9pm ET보다 이른 시작, 우리 기말이 다음 주라 특별 편성) — 딱 한 번 라이브면 이거! · S11 경매=공식 2개(수입등가·입찰셰이딩)만 암기 · 치트시트 2장(4면)", en:"🎯 BEPP final — Pras posted all 4 post-midterm sessions (S8 game theory · S9 utility max/insurance · S10 warranties/asymmetry · S11 auctions → L9–17 fully covered) · ⚠️ Sun 8/9 evening full-review session (earlier than the usual Tue 9pm ET, specially scheduled since finals are next weekend) — if you attend one live session, make it this · S11 auctions = memorize just 2 formulas (revenue equivalence / bid shading) · two cheat sheets (4 sides)", cn:"🎯 BEPP 期末 — Pras 已发全部4节后半课(S8 博弈论·S9 效用最大/保险·S10 保修/信息不对称·S11 拍卖 → L9–17 全覆盖)· ⚠️ 周日 8/9 晚全范围复习课(比平时周二 9pm ET 早,因我们期末在下周特别安排)— 只参加一次直播就选它 · S11 拍卖=只背2个公式(收入等价/出价收缩)· 2张速查表(4面)" },
  ...
  deadlines: [
    {
      date:"2026-08-09", type:"session", course:"BEPP", hard:true, new:true,
      title:{ kr:"BEPP 전범위 리뷰 세션 (일 저녁)", en:"BEPP full-review session (Sun eve)", cn:"BEPP 全范围复习课(周日晚)" },
      detail:{ kr:"⚠️ 일요일 8/9 저녁 · 평소 화 9pm ET보다 이른 시작(기말이 다음 주라 특별 편성) · 후반부(L9–17) 전범위 · 한 학기 딱 한 번 라이브면 이거!", en:"⚠️ Sunday 8/9 evening · earlier than the usual Tue 9pm ET (specially scheduled, finals next weekend) · full post-midterm (L9–17) review · the one live session to attend all term", cn:"⚠️ 周日 8/9 晚 · 比平时周二 9pm ET 早(期末在下周特别安排)· 后半程(L9–17)全范围 · 一学期只参加一次直播就选它" },
    },
    {
  ...
  courses: [
    ...
    },
    {
      ...
      points:[
        ...
        { new:true, kr:"⚠️ **일 8/9 저녁 전범위 리뷰 세션**(평소 화 9pm ET보다 이른 시작, 기말이 다음 주라 특별 편성) — 딱 한 번 라이브면 이거! · 💡 S11 경매=공식 2개(수입등가·입찰셰이딩)만 암기, 나머지는 세팅 인식 · S10=계산 전에 '어떤 개념 묻는지' 먼저 파악(Scoot 2021 자습)", en:"⚠️ Sun 8/9 evening full-review session (earlier than the usual Tue 9pm ET, specially scheduled since finals are next weekend) — if you attend one live session, make it this · 💡 S11 auctions = memorize just 2 formulas (revenue equivalence / bid shading), the rest is setup recognition · S10 = recognize which concept before you compute (Scoot 2021 self-study)", cn:"⚠️ 周日 8/9 晚全范围复习课(比平时周二 9pm ET 早,期末在下周特别安排)— 只参加一次直播就选它 · 💡 S11 拍卖=只背2个公式(收入等价/出价收缩),其余是套路识别 · S10=计算前先判断'考哪个概念'(Scoot 2021 自学)" },
        ...
        { kr:"🗓 기말 준비 도구: 기말 토픽 리스트(8개) + RS5 워크시트+풀이(세션 8/5 완료) · L12–17 'filled in' PDF 전부 게시(게임이론~경매 = 기말 후반부)", en:"🗓 Finals toolkit: finals topic list (8 topics) + RS5 worksheet + solutions (session held 8/5) · L12–17 'filled in' PDFs all posted (game theory through auctions = the final's back half)", cn:"🗓 期末工具: 期末主题清单(8个)+ RS5 练习题+答案(8/5 已结束)· L12–17 'filled in' PDF 已全发(博弈论至拍卖 = 期末后半)" },
      ],
      traps:[
  ...
  actions:[
    { id:"a23", by:"2026-08-09", label:{ kr:"BEPP 전범위 리뷰 세션 참석 (일 8/9 저녁) — 한 학기 딱 한 번이면 이거", en:"Attend the BEPP full-review session (Sun 8/9 evening) — the one to catch all term", cn:"参加 BEPP 全范围复习课(周日 8/9 晚)— 一学期只参加一次就选它" } },
  ...
  canvasMap:[
    {
      ...
      watch:{ kr:"🆕 기말 기출 2021–25(정답 포함)는 이제 Canvas에도 게시(2025 신규) · Pras 세션6–11 전부 게시(6 이부가격·번들링 · 7 과점 · 8 게임이론 · 9 효용/보험 · 10 워런티 · 11 경매) 슬라이드+영상 · 기말 토픽 리스트(8개)·RS5 워크시트 · 미드텀 기출·드롭박스 자료도 유효 · 건너뛴 토픽은 Class Recordings · Inkoo는 공지를 조용히 수정", en:"🆕 Past finals 2021–25 (w/ answers) now on Canvas too (2025 new) · Pras Sessions 6–11 all posted (6 two-part tariffs/bundling · 7 oligopoly · 8 game theory · 9 utility/insurance · 10 warranties · 11 auctions) slides + video · finals topic list (8) · RS5 worksheet · midterm past exams & Dropbox set still valid · skipped topics in Class Recordings · Inkoo edits announcements silently", cn:"🆕 期末真题 2021–25(含答案)现也在 Canvas(2025 新增)· Pras 第6–11课全部已发(6 二部收费/捆绑 · 7 寡头 · 8 博弈论 · 9 效用/保险 · 10 保修 · 11 拍卖)幻灯+视频· 期末主题清单(8个)·RS5 练习题 · 期中真题与 Dropbox 资料仍有效 · 跳过的内容看 Class Recordings · Inkoo 会悄悄改公告" },
    },
    {
      subject:"ACCT",
  ...
  blockWeek: {
    note: { kr:"3–4일 압축 선택과목(보통 0.5 CU, 졸업 19 CU에 포함) · ⚠️ 전공(concentration)엔 Graded만 인정(P/F 불인정) · PHL·SFO 혼합 수강 가능 · Course Match add/drop 진행 중(추가=선착순) — ADD 마감 전 드롭=무기록, 이후=W, P/F 변경은 과목 Day1까지 · ADD 마감은 과목별 블록위크 직전(10/9·11/27·12/11) → 대기줄 늦가을까지 · 대기줄 김(WEMBA51 우선), 'locked'는 정상 · 상세·실라버스·Booklist: Canvas ▸ Files ▸ Classes-Block Week ▸ Term 2 Fall 2026 · 👀 Spring '27 프리뷰: AI in Seattle·Luxury Branding in Paris·PM·Executive Leadership 등 (전체 캘린더: Cohort Comm ▸ Files, \"Cal_51_All_BW_V14\")",
            en:"Electives compressed into 3–4 days (usually 0.5 CU, count toward the 19-CU graduation req) · ⚠️ only Graded courses count toward a major (not P/F) · mix PHL & SFO campuses · Course Match add/drop is open (adds first-come) — drop before the ADD deadline = no record, after = W, P/F changes until Day 1 · ADD deadlines per course right before each block week (10/9 · 11/27 · 12/11) → waitlists alive until late fall · long waitlists (WEMBA51 priority), 'locked' is normal · details, syllabi & Booklist: Canvas ▸ Files ▸ Classes-Block Week ▸ Term 2 Fall 2026 · 👀 Spring '27 peek: AI in Seattle, Luxury Branding in Paris, PM, Executive Leadership & more (full calendar: Cohort Comm ▸ Files, \"Cal_51_All_BW_V14\")",
            cn:"3–4天压缩选修(通常 0.5 CU,计入毕业19 CU)· ⚠️ 专业只认 Graded(P/F 不算)· 可混选 PHL·SFO 校区 · Course Match 加退进行中(加课先到先得)— ADD 截止前退=无记录,之后=W,P/F 可改到 Day1 · ADD 截止按课程各在其 block week 前(10/9·11/27·12/11)→ 候补持续到晚秋 · 等待名单长(WEMBA51 优先),'locked' 属正常 · 详情·大纲·书单: Canvas ▸ Files ▸ Classes-Block Week ▸ Term 2 Fall 2026 · 👀 Spring '27 预告: AI in Seattle·Luxury Branding in Paris·PM·Executive Leadership 等(完整日历: Cohort Comm ▸ Files, \"Cal_51_All_BW_V14\")" },
    options: [
